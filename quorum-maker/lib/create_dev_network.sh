...
function generateSetupConf(){
    ...
    echo 'PUBKEY='$(cat $projectName/node$1/node/qdata/keys/node$1.pub)>> $projectName/node$1/setup.conf
}
...
function generateKeyPair(){
    echo -ne "\n" | crux --generate-keys=node$1 1>>/dev/null
...
function generateEnode(){
    ...
    nodekey=$(cat nodekey)
	bootnode -nodekey nodekey -writeaddress 2 >enode.txt &
	sleep 5
	enode=$(cat enode.txt)
